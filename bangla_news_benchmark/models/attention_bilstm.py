"""
Attention-based Bidirectional LSTM model for Bangla news classification
"""
import torch
...
class AttentionLayer(nn.Module):
    ...
    def __init__(self, hidden_dim):
        """
        Args:
            hidden_dim: Hidden dimension of BiLSTM (will be *2 for bidirectional)
        """
    ...
    def forward(self, lstm_output):
        """
        Args:
            lstm_output: BiLSTM outputs [batch, seq_len, hidden_dim * 2]

        Returns:
            context: Attention-weighted context vector [batch, hidden_dim * 2]
...
class AttentionBiLSTMClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # Apply attention
        context, attention_weights = self.attention(lstm_out)  # context: [batch, hidden_dim * 2]
...
def train_attention_bilstm(data_dict, config):
    """
    ...
    print(f"\nTraining Attention-BiLSTM for {num_epochs} epochs...")
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss_meter.avg < best_val_loss:
            ...
            save_checkpoint(
                ...
                config.get('model_save_path', 'checkpoints/attention_bilstm_best.pt')
            )
    ...
    load_checkpoint(config.get('model_save_path', 'checkpoints/attention_bilstm_best.pt'), model)
    ...
    print("\n" + "="*50)
    print("Attention-BiLSTM Test Results:")
    ...
    return {
        ...
        'model_path': config.get('model_save_path', 'checkpoints/attention_bilstm_best.pt'),
...
if __name__ == "__main__":
    ...
    # Train model
    results = train_attention_bilstm(data_dict, config)
